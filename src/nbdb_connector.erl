...
request(PoolConnector, Number) ->
    io:format("request for ~p extra connections to pool connector ~p~n",
              [Number, PoolConnector]),
...
handle_cast({request, Number}, State) ->
    io:format("~p: got request to open ~p new database connections~n",
              [State#state.pool, Number]),
    OpenFun = State#state.open_fun,
    lists:map(fun(_) -> deliver_connection(State#state.pool, OpenFun()) end,
              lists:seq(1, Number)),
    {noreply, State};
handle_cast({request, Number, From}, State) ->
    io:format("~p: got request from ~p to open ~p new database connections~n",
              [State#state.pool, From, Number]),
    OpenFun = State#state.open_fun,
