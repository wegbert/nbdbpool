...
init({Pool,
    ...
    io:format("(nbdb_pool:init) config: ~n==============~n~p~n==============~n",
              [PoolConfig]),
...
init2(State0) ->
    ...
    State =
        case Min - TC > 0                      %% what todo with pending connections ()
        of
            true ->
                get_extra_connections(Min - TC, State1);
            _ ->
                State1
        end,
    State.
...
handle_cast(init2, State) ->
    {noreply, init2(State)};
handle_cast({add_connection, Connection, From},
            State0 = #state{total_connections = TC, connection_request_len = CRL}) ->
    ...
    State =
        process_unused_connection(Connection,
                                  State0#state{total_connections = TC + 1,
                                               connection_request_len = NewCRL}),
...
handle_cast(UnknownCast, State) ->
    io:format("(~p:~p), handle_cast, unknown cast: ~p~n",
              [?MODULE, State#state.pool, UnknownCast]),
    {noreply, State}.

%%----------------------

handle_call({get_connection, Timeout},
            Client,
            State0 =
                #state{total_connections = TC,
                       max_total = Max,
                       wait_queue = WQ,
                       wait_queue_len = WQL,
                       connection_request_len = CRL}) ->
    %%io:format("(~p:~p), handle_call:get_connection~n",[?MODULE,State0#state.pool]),
    case Timeout < now_milli_secs() of
        true ->
            {reply, timeout, State0};
        _ ->
            State2 =
                case get_from_idle_list(State0) of
                    {ok, Connection, State1} ->
                        deliver_to_client(Connection, Client, State1);
                    _ ->
                        State1 =
                            case TC + CRL < Max of
                                true ->
                                    get_extra_connections(1, State0);
                                _ ->
                                    State0
                            end,
                        State1#state{wait_queue = queue:in({Client, Timeout}, WQ),
                                     wait_queue_len = WQL + 1}
                end,
            {noreply, State2}
    end;
...
handle_call(Request, From, State) ->
    io:format("~p: got call for unknown request ~p from ~p~n",
              [State#state.pool, Request, From]),
    {reply, error, State}.
...
handle_info({'EXIT', Pid, Reason}, State0 = #state{in_use_map = IUM}) ->
    ...
    State =
        case maps:find(Pid, IUM) of
            {ok, Connections} ->
                State1 = State0#state{in_use_map = maps:remove(Pid, IUM)},
...
deliver_to_client(Connection, Client = {Pid, _}, State = #state{in_use_map = IUM}) ->
    ...
    NewEntry =
        case maps:find(Pid,
                       IUM)               %% a single client can have multiple connections in use
        of
            {ok, Value} ->
                [Connection | Value];
            _ ->
                erlang:link(Pid),  %% to get notified when client dies so we can reuse the Connection
                [Connection]
        end,
...
process_unused_connection(Connection,
                          State = #state{wait_queue = WQ, wait_queue_len = WQL}) ->
    case queue:out(WQ) of
        {empty, _} ->
            add_to_idle_list(Connection, State);
        {{value, {Client, Timeout}}, NewQ} ->
            NewWQL = WQL - 1,
            case Timeout < now_milli_secs() of
                true ->
                    process_unused_connection(Connection,
                                              State#state{wait_queue = NewQ,
                                                          wait_queue_len = NewWQL});
                _ ->
                    deliver_to_client(Connection,
                                      Client,
                                      State#state{wait_queue = NewQ, wait_queue_len = NewWQL})
            end
    end.

recalculate_total_connections(State = #state{in_use_map = IUM, idle_list = Idle}) ->
    State#state{total_connections =
                    length(Idle)
                    + maps:fold(fun(_Key, Value, AccIn) -> AccIn + length(Value) end, 0, IUM)}.

get_extra_connections(Number,
                      State = #state{connector = PoolConnector, connection_request_len = CRL}) ->
...
cleanup_connection(Connection,
    ...
    NewIUM =
        maps:filtermap(fun(Client, ConList) ->
    ...
    case (NewIL == IL) and (NewIUM == IUM) of
        ...
        _ ->
            ...
            State#state{idle_list = NewIL,
                        in_use_map = NewIUM,
                        total_connections = TC - 1}
    end.
